<template>
  <div id="createCategory" class="d-flex justify-content-center">
        <div class="card w-50 mt-5">
            <div class="card-header">
                <h3 class="font-weight-bold">Create Category</h3>
            </div>
            <div class="card-body">
                <transition name="fade">
                    <div class="alert alert-success mb-3" v-if="success">{{success}}</div>
                </transition>
                <h5 class="card-title">Name category</h5>
                <form class="login-container" @submit.prevent="createCategory">
                    <div class="form-group mb-4">
                        <input class="form-control" :class="{'is-invalid': errors.name}" type="text" v-model="category.name" placeholder="Name">
                        <div v-if="errors.name" class="invalid-feedback">{{errors.name[0]}}</div>
                    </div>
                    <div class="d-flex justify-content-end">
                        <input class="btn btn-danger" type="reset" value="Clear">
                        <input class="ml-2 btn btn-info" type="submit" value="Create">  
                    </div>
                    
                </form>
            </div>
        </div>
        <Loading v-if="loading"></Loading>
  </div>
</template>

<script>
import Loading from '@/components/custom/Loading'
import { mapActions, mapGetters, mapMutations } from 'vuex'

export default {
    components: { Loading },
    data(){
        return{
            category:{
                name:"",
            },
            loading: false,
            success: "",
        }
    },
    computed:{
        ...mapGetters('categories',['errors'])
    },
    methods:{
        ...mapActions('categories',['create']),
        ...mapMutations('categories',['SET_ERRORS']),
        async createCategory(){
            this.loading = true
            this.success = ""
            this.SET_ERRORS()
            await this.create(this.category)
            if(Object.keys(this.errors).length === 0){
                this.success = "You successfully created category with name "+ this.category.name
                this.category.name = ""
                this.loading = false
            }else{
                this.loading = false
            }
            
        }
    }
}
</script>

<style scoped>
    @import '~@/assets/styles/transition.css';
</style>